 ...
 * 
 * <p>
 * When an initial context is being created, the environment property
 * "java.naming.provider.url" should contain a DNS pseudo-URL (see DnsUrl) or a
 * space-separated list of them. Multiple URLs must all have the same domain
 * value. If the property is not set, the default "dns:" is used.
 * 
...
public class DnsContextFactory implements InitialContextFactory {
  ...
  public Context getInitialContext(Hashtable<?, ?> env) throws NamingException {
    ...
    Context context=urlToContext(getInitCtxUrl(env), env);
    return context;
  }
  ...
  /*
   * "urls" are used to determine the servers, but any domain components are
  ...
  /*
   * Returns all the servers specified in a set of URLs. If a URL has no host
   * (or port), the servers configured on the underlying platform are used if
   * possible. If no configured servers can be found, then fall back to the old
   * behavior of using "localhost". There must be at least one URL.
   */
  ...
  /*
   * Returns true if serversForUrls(urls) would make use of servers from the
   * underlying platform.
   */
  ...
  /*
   * Returns a value for the PROVIDER_URL property (space-separated URL Strings)
   * that reflects the given domain and servers. Each server is of the form
   * "server[:port]". There must be at least one server. IPv6 literal host names
  ...
  /*
   * Reads environment to find URL(s) of initial context. Default URL is "dns:".
   */
   ...
   * 
   * @param input
   *          the input server[:port] list, must not be null
   * @param oneIsEnough
   *          return output once there exists one ok
