...
package com.gmsxo.domains.dns.dnsclient;
...
public class DnsContext extends ComponentDirContext {
  ...
  /**
   * Returns a DNS context for a given domain and servers. Each server is of the
   * form "server[:port]". IPv6 literal host names include delimiting brackets.
   * There must be at least one server. The environment must not be null; it is
   * cloned before being stored.
   */
  ...
  /*
   * Returns a clone of a DNS context, just like DnsContext(DnsContext) but with
   * a different domain name and with parentIsDns set to true.
   */
  ...
  /*
   * Returns a clone of a DNS context. The context's modifiable private state is
   * independent of the original's (so closing one context, for example, won't
   * close the other). The two contexts share <tt>environment</tt>, but it's
   * copy-on-write so there's no conflict.
   */
  ...
  /*
   * Update PROVIDER_URL property. Call this only when environment is not being
   * shared.
   */
  ...
  public Name composeName(Name name, Name prefix) throws NamingException {
    ...
    result = (prefix == prefixC) ? (CompositeName) prefixC.clone() : prefixC;                  // prefixC
                                                                              // is
                                                                              // already
                                                                              // a
                                                                              // clone
  ...
  /*
   * Resolver is not created until needed, to allow time for updates to the
   * environment.
   */
  ...
  /*
   * Returns the fully-qualified domain name of a name given relative to this
   * context. Result includes a root label (an empty component at position 0).
   */
  DnsName fullyQualify(Name name) throws NamingException {
    if (name.isEmpty()) {
      return domain;
    }
    DnsName dnsName = (name instanceof CompositeName) ? new DnsName(name.get(0))                  // parse
                                                                                 // name
  ...
  /*
   * Converts resource records to an attribute set. Only resource records in the
   * answer section are used, and only those that match the classes and types in
   * cts (see classAndTypeMatch() for matching rules).
   */
  ...
  /*
   * Returns true if rrclass and rrtype match some element of cts. A match
   * occurs if corresponding classes and types are equal, or if the array value
   * is ANY. If cts is null, then any class and type match.
   */
  ...
  /*
   * Returns the attribute ID for a resource record given its class and type. If
   * the record is in the internet class, the corresponding attribute ID is the
   * record's type name (or the integer type value if the name is not known). If
   * the record is not in the internet class, the class name (or integer class
   * value) is prepended to the attribute ID, separated by a space.
   * 
   * A class or type value of ANY represents an indeterminate class or type, and
   * is represented within the attribute ID by "*". For example, the attribute
   * ID "IN *" represents any type in the internet class, and "* NS" represents
   * an NS record of any class.
   */
  ...
  /*
   * Returns the class and type values corresponding to an attribute ID. An
   * indeterminate class or type is represented by ANY. See toAttrId() for the
   * format of attribute IDs.
   * 
   * @throws InvalidAttributeIdentifierException if class or type is unknown
   */
  ...
  /*
   * Returns an array of the classes and types corresponding to a set of
   * attribute IDs. See toAttrId() for the format of attribute IDs, and
   * classAndTypeMatch() for the format of the array returned.
   */
  ...
  /*
   * Returns the most restrictive resource record class and type that may be
   * used to query for records matching cts. See classAndTypeMatch() for
   * matching rules.
   */
   ...
   * 
   * Any access to zoneTree that walks the tree, whether it modifies the tree or
   * not, is synchronized on zoneTree. [%%% Note: a read/write lock would allow
   * increased concurrency.] The depth of a ZoneNode can thereafter be accessed
   * without further synchronization. Access to other fields and methods should
   * be synchronized on the node itself.
   * 
   * A zone's contents is a NameNode tree that, once created, is never modified.
   * The only synchronization needed is to ensure that it gets flushed into
   * shared memory after being created, which is accomplished by
   * ZoneNode.populate(). The contents are accessed via a soft reference, so a
   * ZoneNode may be seen to be populated one moment and unpopulated the next.
   */

  /*
   * Returns the node in the zone tree corresponding to a fully-qualified domain
   * name. If the desired portion of the tree has not yet been populated or has
   * been outdated, a zone transfer is done to populate the tree.
   */
    ...
    if (znode != null) {
      ...
      if (topOfZone != null) {
        ...
        if ((nnode != null) && !nnode.isZoneCut()) {
          ...
          zone = (DnsName) fqdn.getPrefix(znode.depth() + 1);      // +1 for root
  ...
  /*
   * Does a zone transfer to [re]populate a zone in the zone tree. Returns the
   * zone's new contents.
   */
  ...
  /*
   * Determine if a ZoneNode's data is current. We base this on a comparison
   * between the cached serial number and the latest SOA record.
   * 
   * If there is no SOA record, znode is not (or is no longer) a zone:
   * depopulate znode and return false.
   * 
   * Since this method may perform a network operation, it is best to call it
   * with znode unlocked. Caller must then note that the result may be outdated
...
/*
 * A pairing of a resource record class and a resource record type. A value of
 * ANY in either field represents an indeterminate value.
 */
class CT {
  int rrclass;
  int rrtype;
 ...
 * 
 * Nodes that have children or that are zone cuts are returned with classname
 * DirContext. Other nodes are returned with classname Object even though they
 * are DirContexts as well, since this might make the namespace easier to
 * browse.
 */
